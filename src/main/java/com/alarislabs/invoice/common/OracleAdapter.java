...
import com.alarislabs.invoice.payments.Payable;
...
public class OracleAdapter {
    ...
    Map<String, String> getSystemParams() {
        ...
        try {
            ...
            String query = "begin ? := invoice.prv_rest_api.get_param_list; end;";
            ...
            Log.error("Unsuccessful execution of invoice.prv_rest_api.param_list: " + e.getMessage());
            return null;
        } finally {
            try {
                if (rs != null) rs.close();
    ...
    Response getResponseJson(JsonObject inputJson, boolean firstAttempt, List<String> rowsArray, boolean addTotalCountHeader) throws Exception {
        ...
        CallableStatement stmt = null;
        ResultSet rs = null;
        String query = "begin ? := invoice.prv_rest_api.get_response(?, ?, ?); end;";

        try {

            ArrayDescriptor desc = ArrayDescriptor.createDescriptor("INVOICE.PRV_VARCHAR4000_TABLE_T", oraConn);
            ...
            if (outParamsJson != null) {
                ...
                if (outParamsJson.has(HTTP_ERROR_CODE)) {
                ...
                } else if (outParamsJson.has(AUTH_LOGIN) && outParamsJson.has(AUTH_CAR_ID)) {
                    ...
                    outParamsJson.addProperty("token", Auth.getToken(
                            ...
                            outParamsJson.get(AUTH_CAR_ID).getAsInt()
                        )
                    );
    ...
    Response postPayment(Payable payment, boolean firstTry) {

        Log.info(String.format("New %s payment arrived. Parameters: %s", payment.getPaymentMethod(), payment.getPaymentDetails()));

        if (!payment.isVerified()) {
            Log.warn("Payment verification failed");
            return Response.newErrInstance("Payment verification failed", HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            Log.error("Error connecting to Oracle while processing payment from PayPal: ", e);
            return Response.newErrInstance("Oracle connection failed: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
        ...
        try {
            ...
            CallableStatement stmt = null;
            String query = "begin ? := invoice.prv_rest_api.post_payment(?, ?); end;";

            try {
                stmt = oraConn.prepareCall(query);
                stmt.registerOutParameter(1, OracleTypes.VARCHAR);
                stmt.setString(2, payment.getPaymentMethod());
                stmt.setString(3, payment.getPaymentDetails());
                stmt.execute();

                String outParams = (String) stmt.getObject(1);
                oraConn.commit();
            ...
            Log.error("Error processing " + payment.getPaymentMethod() + " payment: " + e.getMessage());

            if (firstTry && (e.getErrorCode() == 4068 //existing state of packages has been discarded
                    || e.getErrorCode() == 4061 //existing state of  has been invalidated
                    || e.getErrorCode() == 25408)) { //can not safely replay call)
                Log.info("Retrying processing payment");
                return postPayment(payment, false);
            } else {
                return Response.newErrInstance(translateErrorMsg(e.getMessage()), HttpStatus.BAD_REQUEST);
            }
    ...
    private String translateErrorMsg(String errorMsg) {
        ...
        try {
            ...
            CallableStatement stmt = null;
            String query = "begin ? := invoice.prv_rest_api.get_translated_error_msg(?); end;";
