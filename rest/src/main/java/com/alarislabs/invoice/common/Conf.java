...
public class Conf {
    ...
    static boolean reload() {
        ...
        try {
            ...
            if (!Utils.hasValue(configPath)) configPath = "/etc/invoice/restman.conf";
            ...
            for(String pn : newValues.keySet()) {
                ...
                if (!systemParams.containsKey(pn) || !newValues.get(pn).equals(systemParams.get(pn))) {
                    ...
                    if (pn.equals(RESPONSE_RECORD_COUNT_LIMIT_PN)) {
                        responseRecordCountLimit = Utils.nvl(Integer.parseInt(systemParams.get(RESPONSE_RECORD_COUNT_LIMIT_PN)), 10000);
                    }
    ...
    static int getResponseRecordCountLimit() {
        return responseRecordCountLimit;
    }
